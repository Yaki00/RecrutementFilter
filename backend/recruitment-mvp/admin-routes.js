...
function registerAdminRoutes(app, { db, adminConfig, sessionStore = createAdminSessionStore() }) {
  ...
  function getAttemptRecord(ip) {
    return new Promise((resolve, reject) => {
      db.get(
        `SELECT ip, failed_attempts, banned_until FROM admin_login_attempts WHERE ip = ?`,
        [ip],
        (err, row) => {
          if (err) reject(err);
  ...
  function saveAttemptRecord(ip, failedAttempts, bannedUntil) {
    return new Promise((resolve, reject) => {
      db.run(
        `
          INSERT INTO admin_login_attempts (ip, failed_attempts, banned_until, updated_at)
          VALUES (?, ?, ?, ?)
        ...
        `,
        [ip, failedAttempts, bannedUntil, new Date().toISOString()],
        (err) => {
          if (err) reject(err);
          else resolve();
        }
    ...
    try {
      ...
      if (loginState.banned) {
        return res.status(423).json({
          ok: false,
          error: "Accès temporairement bloqué après 3 tentatives incorrectes.",
      ...
      if (!verifyAdminPassword(password, adminConfig)) {
        const update = getFailedAttemptUpdate(record?.failed_attempts || 0);
        await saveAttemptRecord(ip, update.failedAttempts, update.bannedUntil);
  ...
  app.post("/api/admin/logout", requireAdminAuth, (req, res) => {
    sessionStore.revoke(req.adminToken);
    res.json({ ok: true });
  });
  ...
  app.get("/api/admin/participants", requireAdminAuth, (_, res) => {
  ...
  app.get("/api/admin/kpis", requireAdminAuth, (_, res) => {
    fetchParticipants((error, participants) => {
      if (error) {
  ...
  app.post("/api/admin/purge", requireAdminAuth, async (_, res) => {
    try {
  ...
  app.post("/api/admin/erase-candidate", requireAdminAuth, async (req, res) => {
